...
public class MMCommand extends BaseCommand {
    ...
    public void create(Player player, String id, EntityType type) {
        if (plugin.mobRegistry.get(id) != null) {
        ...
        mob.setHealth(new MobHealth(null, 10)); // Default 10 health
        // Default example rewards
        List<cz.miniomega.mobs.rewards.Reward> exampleRewards = new LinkedList<>();
        exampleRewards.add(new cz.miniomega.mobs.rewards.top.TopReward(
                "reward_1",
                cz.miniomega.mobs.util.range.NumberRange.parse("1").get(),
                cz.miniomega.mobs.rewards.commands.RewardCommands.parse("RANDOM", List.of("100;say %player% is first!"))
        ));
        exampleRewards.add(new cz.miniomega.mobs.rewards.top.TopReward(
                "reward_2",
                cz.miniomega.mobs.util.range.NumberRange.parse("2").get(),
                cz.miniomega.mobs.rewards.commands.RewardCommands.parse("RANDOM", List.of("100;say %player% is second!"))
        ));
        exampleRewards.add(new cz.miniomega.mobs.rewards.top.TopReward(
                "reward_3",
                cz.miniomega.mobs.util.range.NumberRange.parse("3").get(),
                cz.miniomega.mobs.rewards.commands.RewardCommands.parse("RANDOM", List.of("100;say %player% is third!"))
        ));

        mob.setRewards(new MobRewards(plugin, new LinkedList<>(), exampleRewards));
        mob.setMessages(new MobMessages("&aMob %player% was defeated!"));
    ...
    public void delete(Player player, String id) {
        MineMob mob = plugin.mobRegistry.get(id);
        if (mob == null) {
            player.sendMessage(Colors.colorize("&cMob with that ID does not exist!"));
            return;
        }

        plugin.mobRegistry.unregister(mob);
    ...
    public void setHologramLine(Player player, String id, int line, String text) {
        ...
        if (line < 1 || line > lines.size()) {
            player.sendMessage(Colors.colorize("&cInvalid line nummer!"));
            return;
        }
