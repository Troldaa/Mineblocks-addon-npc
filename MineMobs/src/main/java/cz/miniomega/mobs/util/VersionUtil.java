package cz.miniomega.mobs.util;

import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import cz.miniomega.mobs.MineMobsPlugin;
import cz.miniomega.mobs.util.result.Result;
...
public class VersionUtil {
    ...
    public static CompletableFuture<Optional<String>> shouldUpdate(MineMobsPlugin plugin) {
    ...
    public static CompletableFuture<Result<File, String>> downloadLatest(MineMobsPlugin plugin) {
        return getPluginInformation().thenApplyAsync(o -> {
            ...
            try {
                ...
                pluginFile = (File) getFileMethod.invoke(plugin);
            } catch (Exception e) {
                e.printStackTrace();
            }

            if (pluginFile == null) return Result.error("Could not find plugin file!");

            File file = new File(pluginFile.getParentFile(), "MineMobs-"+ version +".jar");
